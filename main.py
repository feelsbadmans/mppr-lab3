...
def mnist_train(model):
    ...
    mnist_path = 'mnist/'
    X_train = idx2numpy.convert_from_file(mnist_path + 'emnist-byclass-train-images-idx3-ubyte')
    y_train = idx2numpy.convert_from_file(mnist_path + 'emnist-byclass-train-labels-idx1-ubyte')

    X_test = idx2numpy.convert_from_file(mnist_path + 'emnist-byclass-test-images-idx3-ubyte')
    y_test = idx2numpy.convert_from_file(mnist_path + 'emnist-byclass-test-labels-idx1-ubyte')

    X_train = np.reshape(X_train, (X_train.shape[0], 28, 28, 1))
    X_test = np.reshape(X_test, (X_test.shape[0], 28, 28, 1))

    print(X_train.shape, y_train.shape, X_test.shape, y_test.shape, len(mnist_labels))
    ...
    model.fit(X_train, x_train_cat, validation_data=(X_test, y_test_cat), callbacks=[learning_rate_reduction], batch_size=32, epochs=30)
...
if __name__ == "__main__":
    ...
    # model = mnist_model()
    # mnist_train(model)
    # model.save('mnist_letters3.h5')

    model = keras.models.load_model('mnist_letters_2.h5')
